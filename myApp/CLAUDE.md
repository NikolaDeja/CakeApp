...
- The `RootStackParamList` entry for `Recipe` in [App.tsx](App.tsx) declares params `{ cake, layer1, layer2, layer3 }`, but `CreateCakeScreen` navigates with a different, richer param set (`cakeType`, `cakeType2`, `layers: string[]`, `selectedAllergens`, `selectedDecorations`, `outerLayer`, `selectedPortionVsSize`, `portionSize`, `selectedShape`, `layerCount`), and `RecipeScreen` reads yet a third shape (`cakeType`, `layer1`, `layer2`, `layer3`). The types, the sender, and the receiver are three different contracts — fix all three together.
